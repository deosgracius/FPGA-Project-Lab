module line_follower(
    ...
    // Turn timing
    reg [19:0] turn_timer = 20'd0;  // Timer for turn duration
    parameter TURN_DURATION = 20'd100000;  // Adjust this value based on your clock frequency
    reg turn_timing = 1'b0;  // Flag for turn timing phase
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            if (turn_timing) begin
                // During initial turn phase - ignore sensors and count time
                stateL <= 2'b10;   // Left motor reverse
                stateR <= 2'b01;   // Right motor forward
                
                if (turn_timer >= TURN_DURATION) begin
                    turn_timing <= 1'b0;  // Switch to sensor-based completion
                end else begin
                    turn_timer <= turn_timer + 1;
                end
            end else begin
                // After time-based turn, wait for line detection
            ...
            if (intersection_detected) begin
                case (turn_counter)
                    3'b000: begin  // First intersection - turn left
                        ...
                        stateR <= 2'b01;
                        turning_left <= 1'b1;
                        turn_timing <= 1'b1;  // Start timing the turn
                        turn_timer <= 20'd0;  // Reset timer
